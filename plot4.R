...
#Setting the parameters for a matrix with 2 rows and 2 columns
par(mfcol=c(2,2))
# Top left corner
with(hpc_subset, plot(hpc_subset$Time,as.numeric(as.character(hpc_subset$Global_active_power)),type="l",xlab="",ylab="Global Active Power")) 
# Bottom left corner
...
legend("topright", bty = "n", lty=1, col=c("black", "red", "blue"), legend=c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), cex=0.9)
# Top right corner
with(hpc_subset, plot(hpc_subset$Time, as.numeric(as.character(hpc_subset$Voltage)), type="l",xlab="datetime",ylab="Voltage"))
#Bottom left corner
with(hpc_subset, plot(hpc_subset$Time, as.numeric(as.character(hpc_subset$Global_reactive_power)),type="l", xlab="datetime",ylab="Global_reactive_power", col="black", lwd=0.2))
dev.off()